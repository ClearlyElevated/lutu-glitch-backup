...
class warningHandler {
  static process(client, warnedMember, warnAuthor, guild) {
    const muteUser = async (client, member, duration) => {
      ...
      setTimeout(async () => {
        if (!member.roles.has(muteRole.id)) return;
        await logHandler.sendLog(client, { channel: "NONE" }, guild.id, 9, {
          moderator: client.user,
          user: warnedMember,
          reason: "Temporary Mute Complete"
        });
        try {
    ...
    const banUser = async (client, member) => {
      ...
      try {
        await member.ban(
          "Banned for reaching the configured warning limit in specific serve"
        );
      } catch (e) {
        return;
      }
    ...
    const kickUser = async (client, member) => {
      ...
      try {
        await member.kick(
          "Kicked for reaching as many warning as setted in guild settings for a user to get kicked."
        );
      } catch (e) {
        return;
      }
          ...
          async (errr, user) => {
            ...
            for (const punishment of settings.punishments) {
              if (user.infractions === punishment.nr) {
                ...
                if (instr[0] === "mute")
                  await muteUser(client, warnedMember, instr[1]);
                ...
                if (instr[0] === "ban")
                  await logHandler.sendLog(
                    ...
                    1,
                    {
                      moderator: warnAuthor,
                      user: warnedMember,
                      reason: `Reached ${user.infractions} Infractions`
                    }
                  );
                if (instr[0] === "kick")
                  await logHandler.semdLog(
                    client,
                    { channel: "NONE" },
                    guild.id,
                    2,
