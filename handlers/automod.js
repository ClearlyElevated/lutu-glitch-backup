...
const sightengine = require("sightengine")("1822450516", "iAqeHHs9YcKSz42oqFh2");
      ...
      async (err, u) => {
        ...
        const antiSpam = async (client, message) => {
          ...
          const giveWarnUser = async m => {
            ...
            m.channel.send(
              `<@!${m.author.id}>, has been given \`1\` warning for spamming.`
            );
          ...
          const warnUser = async m => {
            ...
            m.channel
              .send(`<@${m.author.id}>, Please stop spamming.`)
          ...
          if (message.author.id !== client.user.id) {
            ...
            if (
              msgMatch == maxDuplicatesBan &&
              !givedWarn.includes(message.author.id)
            ) {
        ...
        const antiLink = async (client, message) => {
          ...
          const warnIt = async () => {
            ...
            message.channel.send(
              `<@!${message.author.id}>, has been given \`1\` infraction for posting links.`
            );
        ...
        const antiInvite = async (client, message) => {
          ...
          const warnTheUser = async () => {
            ...
            message.channel.send(
              `<@!${message.author.id}>, has been given \`1\` infraction for posting discord invitation links.`
            );
            u.infractions = u.infractions + 1;
            await u.save().catch(e => client.logger.log(e, "error"));
            warningHandler.emit(
        ...
        const maxLines = async (client, message, amount) => {
          ...
          if (maxLines.length > amount) {
            message.delete().catch(e => e);
            if (message.guild.settings.moderationLogs.toLowerCase() === "on") {
              const Logger4 = new logHandler({
                client: client,
                case: "automod",
                guild: message.guild.id,
                member: message.author,
                reason: "Exceeding limit of maximum lines per message."
              });
              Logger4.send().then(t => Logger4.kill());
            }
            message.channel.send(
              `<@!${message.author.id}>, has been given \`1\` infraction because its message contained ${maxLines.length} new lines.`
            );
            ...
            try {
              message.author.send(
                `Here its your message if you wish to edit it:\n${message.content}`
              );
            } catch (e) {
              return;
            }
          ...
          if (
            ...
            if (
              !message.channel
                ...
                .toArray()
                .includes("MENTION_EVERYONE")
            ) {
              message.delete().catch(e => e);
              if (
                message.guild.settings.moderationLogs.toLowerCase() === "on"
              ) {
                const Logger5 = new logHandler({
                  client: client,
                  case: "automod",
                  guild: message.guild.id,
                  member: message.author,
                  reason: "Atempt to mention @everyone/@here."
                });
                Logger5.send().then(t => Logger5.kill());
              }
              message.channel.send(
                `<@!${message.author.id}>, has been given \`1\` infraction for attempt to mention at here/ at everyone.`
              );
        ...
        const maxMentions = async (client, message, amount) => {
          ...
          if (totalMentions > amount) {
            message.delete().catch(e => e);
            if (message.guild.settings.moderationLogs.toLowerCase() === "on") {
              const Logger6 = new logHandler({
                client: client,
                case: "automod",
                guild: message.guild.id,
                member: message.author,
                reason: "Exceeding limit of maximum mentions per message."
              });
              Logger6.send().then(t => Logger6.kill());
            }
            message.channel.send(
              `<@!${message.author.id}>, has been given \`1\` infraction for mass pinging.`
            );
        ...
        const antiBad = async (client, message) => {
          ...
          const sueAuthor = async (client, message) => {
            ...
            message.delete().catch(e => e);
            if (message.guild.settings.moderationLogs.toLowerCase() === "on") {
              const Logger7 = new logHandler({
                client: client,
                case: "automod",
                guild: message.guild.id,
                member: message.author,
                reason: "Sending bad words in messges."
              });
              Logger7.send().then(t => Logger7.kill());
            }
            message.channel.send(
              `<@!${message.author.id}>, has been given \`1\` infraction for saying bad words!`
            );
          ...
          for (const arg of args) {
            if (stat === false) {
              if (badWords.includes(arg.toLowerCase()))
        ...
        const antiNsfw = async (client, message) => {
          ...
          const punishUser = async (client, message) => {
            ...
            message.delete().catch(e => e);
            if (message.guild.settings.moderationLogs.toLowerCase() === "on") {
              const Logger8 = new logHandler({
                client: client,
                case: "automod",
                guild: message.guild.id,
                member: message.author,
                reason: "Sending inappropriate attachments."
              });
              Logger8.send().then(t => Logger8.kill());
            }
            message.channel.send(
              `<@!${message.author.id}>, has been given \`1\` infraction for posting NSFW content!`
            );
        ...
        if (settings.maxMentions > 0)
          maxMentions(client, message, settings.maxMentions);
