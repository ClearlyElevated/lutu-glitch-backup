...
module.exports = client => {
  ...
  app.get("/discord", (req, res) => {
    res.redirect("https://discord.gg/7yX39K");
  });
  ...
  app.get("/invite", (req, res) => {
    res.redirect(
      "https://discordapp.com/api/oauth2/authorize?client_id=584279261368811520&permissions=8&redirect_uri=http%3A%2F%2Fwww.lutu.botcord.site%2Fcallback&response_type=code&scope=bot%20identify%20guilds"
    );
  });
  ...
  app.get(
    ...
    (req, res) => {
      session.us = req.user;
      if (
        req.user.id === client.appInfo.owner.id ||
        client.config.admins.includes(req.user.id)
      ) {
  ...
  app.get("*", (req, res, next) => {
    if (!req.session.isAdmin && client.dashboardMaintenance === true)
  ...
  app.get("/staff", (req, res) => {
    renderTemplate(res, req, "staff", { md });
  });
  ...
  app.get("/stats/:guildID", checkAuth, async (req, res) => {
    ...
    try {
      guildBans = await guild.fetchBans();
    } catch (e) {
      guildBans = { size: "??" };
    }
    try {
  ...
  app.post("/report", checkAuth, async (req, res) => {
    ...
    if (!reason)
      return renderTemplate(res, req, "report.ejs", {
    ...
    if (banned)
      return renderTemplate(res, req, "report.ejs", {
        error: "User is already on ban list.",
        success: null
      });

    const stillAwaitCase = await pendingBan.findOne({
      reportedByID: req.user.id
    });
    if (stillAwaitCase)
      return renderTemplate(res, req, "report.ejs", {
        error: "You cannot have more than one reports awaiting at a time.",
        success: null
      });
    ...
    renderTemplate(res, req, "report.ejs", {
      success: "User has been reported.",
      error: null
    });
  });
  ...
  app.get("/admin/:guildID/warp", checkAuth, async (req, res) => {
    ...
    const invite = await channel
      ...
      .catch(e =>
        client.users
          .get(req.user.id)
          .send(`Couldn't send an invite because ${e}`)
      );
    client.users
      .get(req.user.id)
      .send(
        `Invite Generated to \`${guild.name}\` is https://discord.gg/${invite.code}/.`
      );
    res.redirect("/admin");
  });
