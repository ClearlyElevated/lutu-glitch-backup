...
module.exports = class {
  ...
  async run(message) {
    function getDmLvl(message, client) {
      if (message.author.id === client.appInfo.owner.id) {
        return 10;
      } else if (client.config.admins.includes(message.author.id)) {
        return 9;
      } else {
        return 0;
      }
...
Maintained By: **__ClearlyElevated__**
        `
        )
        .setColor("RANDOM")
        .setTimestamp();
      return message.channel.send(
        `Hey there, my prefix is \`${guildSettings.prefix}\`.`,
        helpEmbed
      );
    }
    ...
    const level =
      message.channel.type === "text"
        ? await this.client.permlevel(message)
        : getDmLvl(message, this.client);
    ...
    if (message.guild && level < 2) {
      if (
        message.member &&
        !guildSettings.ignoredUsers.includes(message.author.id)
      ) {
        if (!guildSettings.ignoredChannels.includes(message.channel.id)) {
          if (
            !message.member.roles.some(r =>
              guildSettings.ignoredRoles.includes(r.id)
            )
          ) {
    ...
    if (cmd && usrs.includes(message.author.id))
      return reply(
        "Seems like you can't access this. You've been banned by one of bot admins for more details regarding your case and appeal info, join our support server"
      );
        ...
        if (command === selfrole.name) {
          if (message.member.roles.find(role => role.id === selfrole.ID)) {
            message.member.roles.remove(
              message.guild.roles.find(role => role.id === selfrole.ID)
            );
    ...
    if (!cmd) return;
    if (level < 9 && this.client.cmdMaintenance === true)
      return reply(
        "We are currently undergoing a maintenance we'll be back soon."
      );
    if (cmd.conf.enabled === false)
      return reply(
        "This command is currently globally disabled. Visit our dashboard at https://lutu.botcord.site."
      );
    ...
    if (
      message.author.id !== "542058542853521461" &&
      message.author.id !== ""
    ) {
      if (!timestamps.has(message.author.id)) {
        ...
        if (now < expirationTime) {
          ...
          return reply(
            `Slow it down dude. You have to wait ${timeLeft.toFixed(
              1
            )} seconds before using \`${cmd.help.name}\` again.`
          );
        }
...
▫ Required Permission Level: ${this.client.levelCache[cmd.conf.permLevel]} - ${
          cmd.conf.permLevel
        }
▫ Your Permission Level: ${level} - ${
    ...
    if (level < this.client.levelCache[cmd.conf.permLevel])
      return reply(noPermEmbed);

    message.author.permLevel = level;

    const timestamp = `[${moment().format("YYYY-MM-DD HH:mm:ss")}]:`;
    const content = `${timestamp} ${message.author.tag} (ID:${
      message.author.id
    }) ran ${cmd.help.name} in ${
      message.guild ? message.guild.name : "DMs"
    } (ID: ${message.guild ? message.guild.id : "0"})
...
`;
    ...
    this.client.channels
      .get(this.client.config.commandLogChannel)
      .send(
        `**${cmd.help.name}** was used by \`${message.author.tag}\` (ID: ${
          message.author.id
        }) in guild \`${
          message.guild ? message.guild.name : "Direct Message"
        }\` (ID: ${
          message.guild ? message.guild.id : "583972685990854676"
        }), channel \`${message.channel.name}\` (ID: ${
          message.channel.id
        }).\nMessage content: \n\`\`\`${message.content}\`\`\``
      );
    try {
      ...
      this.client.channels
        .get(this.client.config.errorChannel)
        .send(
          `An internal error occured while running \`${cmd.help.name}.js\`.\n\`\`\`xl\n${e}\`\`\``
        );
    }
  }
};
