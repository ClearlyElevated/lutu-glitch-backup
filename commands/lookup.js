...
class Lookup extends Command {
  constructor (client) {
    super(client, {
      name: "lookup",
      description: "Check the `Ban Status` of a user or yourself.",
      category: "Ban List",
      usage: "[user]",
      enabled: true,
      guildOnly: true,
  ...
  async run (message, args, level, reply) { // eslint-disable-line no-unused-vars
    ...
    if (!usr) return reply("<a:aRedTick:584866618039861249> Looks like you didn't specify a valid user.");

    if (usr === args[0]) {
      try {
        await this.client.fetchUser(args[0]);
      } catch (e) {
        return reply("<a:aRedTick:584866618039861249> Looks like you didn't specify a valid user.");
      }
    }
    ...
    Bans.findOne({
      ...
      if (!u) {
        embed.addField("[Status]:", "<a:aGreenTick:584866617780076576> Not Banned");
      }

      if (u) {
        embed.addField("[Status]:", "<a:aRedTick:584866618039861249> Banned");
