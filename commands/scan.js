...
class Scan extends Command {
  ...
  async run (message, args, level, reply) { // eslint-disable-line no-unused-vars
    ...
    const mssg = await reply("<a:waiting:588455333119131711> Scanning all of the members . This might take a while depending on the size of your server. Estimated time: **" + (message.guild.memberCount * 70 / 1000) + "** seconds.");
    if (mode === "warn") {
      const arr = [];
