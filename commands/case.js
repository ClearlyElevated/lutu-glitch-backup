...
class Case extends Command {
  ...
  async run(message, args, level, reply) {
    ...
    if (!complaint || complaint === null)
      return reply("This complaint does not exist.");
    if (!args[1])
      return reply(
        "You must pick an option wheter to reject it or approve it."
      );
    ...
    if (args[1].toLowerCase() === "approve") {
      ...
      this.client.users
        .get(complaint.reportedByID)
        .send(
          `Your report with case id **${complaint.caseID}** has been approved by **${message.author.tag} <a:aGreenTick:584866617780076576>**.`
        );
      const reportedUser = await this.client.users.fetch(complaint.reportedID);
      const reportAuthor = await this.client.users.fetch(
        complaint.reportedByID
      );

      const approvedEmbed = new Discord.MessageEmbed()
        .setTitle("Report Approved <a:aGreenTick:584866617780076576>")
        .addField(
          "[User]:",
        ...
        .addField(
          "[Complaint]:",
          `▫ Complaint Author: ${reportAuthor.tag}\n▫ Complaint Author ID: ${
            reportAuthor.id
          }\n▫ Complaint Reason: ${
            complaint.caseReason
      ...
      this.client.channels
        .get(this.client.config.reportApprovedEmbedChannel)
        .send(approvedEmbed);
      ...
      this.client.users
        .get(complaint.reportedByID)
        .send(
          `Your report with case id **${complaint.caseID}** has been rejected by **${message.author.tag}** because **${reason}**.`
        );
      const reportedUser = await this.client.users.fetch(complaint.reportedID);
      const reportAuthor = await this.client.users.fetch(
        complaint.reportedByID
      ...
      const rejectedEmbed = new Discord.MessageEmbed()
        .setTitle("Report Rejected <a:aRedTick:584866618039861249>")
        .addField(
          "[User]:",
          `▫ Username: ${reportedUser.username}\n▫ Tag: ${reportedUser.tag}\n▫ ID: ${reportedUser.id}`
        )
        ...
        .setTimestamp();
      this.client.channels
        .get(this.client.config.reportRejectedEmbedChannel)
        .send(rejectedEmbed);
