  ...
  async run(message, args, level, reply) {
    ...
    bans.findOne(
      ...
      async (err, r) => {
        ...
        const q = await this.client.awaitReply(
          message,
          `Do you want to appeal case **${r.caseID}** for \`${r.caseReason}\`? (\`yes\`/\`no\`)`
        );
        if (q === false) return reply("Prompt timed out!");

        if (q.toLowerCase() === "yes") {
          const prof = await this.client.awaitReply(
            message,
            "Please send links of photos/videos (photos better) demonstrating that you're not guilty. Please submit each link separated by a comma and and a space. Ex: Link1, Link2, Link3, Link4 You may only submit up to 4 proof links"
          );
          ...
          await reply(
            "This is a preview of the appeal. Do you want to submit it? (`yes`/`no`)"
          );
          ...
          if (conf.toLowerCase() === "yes") {
            await this.client.channels
              .get(this.client.config.appealEmbedChannel)
              .send(previewEmbed);
          } else {
